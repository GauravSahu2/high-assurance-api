...
def test_aws_secret_manager_fatal_error():
    ...
    with patch("boto3.client") as mock_boto:
        ...
        try: main._load_secret("dummy", "fallback")
...
def test_database_commit_rollbacks():
    ...
    with patch("sqlalchemy.orm.Session.commit", side_effect=Exception("Simulated DB Crash")):
        ...
        client.post("/transfer", json={"to_user":"u", "amount": 10}, headers={"Authorization": f"Bearer {token}"})
...
def test_redis_connection_drops():
    ...
    with patch("redis.Redis.get", side_effect=redis.RedisError("Simulated Redis Crash")), \
         ...
         token = generate_jwt("admin")
         client.post("/transfer", json={"to_user":"u", "amount": 10}, headers={"Authorization": f"Bearer {token}", "Idempotency-Key": "123"})
