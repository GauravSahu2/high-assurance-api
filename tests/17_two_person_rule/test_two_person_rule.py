...
def verify_deployment_authorization(modified_files, approvers):
    ...
    if touches_critical_code:
        senior_approvers = [
            user for user, role in approvers.items()
            if role in ["Senior", "Lead", "Staff"]
        ]
...
def test_deployment_authorization():
    ...
    assert is_authorized, \
        "CRITICAL FAIL: Pipeline blocked a properly authorized deployment."
...
def test_codeowners_file_exists():
    ...
    assert os.path.exists(codeowners_path), \
