...
class TestChangeManagement:
    ...
    def test_codeowners_file_exists(self):
        ...
        path = os.path.join(
            os.path.dirname(__file__), "..", "..", ".github", "CODEOWNERS"
        )
        assert os.path.exists(path), \
            "SOC 2 VIOLATION: CODEOWNERS file must exist in .github/"
        with open(path) as f:
            content = f.read()
    ...
    def test_gitops_deployment_pipeline_exists(self):
        ...
        assert os.path.exists(gitops_path), \
            "GitOps deployment pipeline must exist"
    ...
    def test_argocd_gitops_manifest_exists(self):
        ...
        path = os.path.join(
            os.path.dirname(__file__), "..", "..", "argocd-app.yaml"
        )
    ...
    def test_two_person_rule_test_exists(self):
        ...
        path = os.path.join(
            os.path.dirname(__file__), "..", "17_two_person_rule",
            "test_two_person_rule.py"
        )
        assert os.path.exists(path), \
            "Two-person rule test must exist"
