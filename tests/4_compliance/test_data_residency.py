...
class TestDataResidency:
    ...
    def test_aws_region_is_configurable(self):
        ...
        assert (
            "AWS_DEFAULT_REGION" in source or "region_name" in source
    ...
    def test_database_url_does_not_leak_region(self, client):
        ...
        for indicator in cloud_indicators:
            assert indicator not in body, f"Health endpoint leaks cloud region info: {indicator}"
    ...
    def test_error_responses_do_not_leak_infrastructure(self, client):
        ...
        body = res.get_data(as_text=True)
        infra_terms = ["amazonaws", "azure", "gcp", "localhost:5432", "redis://", "postgresql://"]
        for term in infra_terms:
            assert term not in body, f"Error response leaks infrastructure info: {term}"
