...
def test_gdpr_erasure_endpoint_admin(client, token_factory):
    ...
    res = client.delete("/api/users/user_2/data", headers={"Authorization": f"Bearer {admin_token}"})
...
def test_gdpr_erasure_endpoint_self(client, token_factory):
    ...
    res = client.delete("/api/users/user_1/data", headers={"Authorization": f"Bearer {user_token}"})
    assert res.status_code == 200
...
def test_gdpr_erasure_endpoint_forbidden(client, token_factory):
    ...
    user_token = token_factory("user_1", "user")
    res = client.delete("/api/users/user_2/data", headers={"Authorization": f"Bearer {user_token}"})
...
def test_gdpr_erasure_endpoint_not_found(client, token_factory):
    ...
    admin_token = token_factory("admin", "admin")
    res = client.delete("/api/users/non_existent/data", headers={"Authorization": f"Bearer {admin_token}"})
